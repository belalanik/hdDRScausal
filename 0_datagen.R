...
## Binary exposure
# tb.infection: Tuberculosis infection status 
...
# Exposure
tb.infection <- rbinom(n, 1, prob = plogis(-3.35 + 0.05*age - 0.5*sex + 0.4*comorbidity))
vars <- data.frame(tb.infection, age, sex, comorbidity)

# Generating data
simdat <- simsurv(dist = "weibull", lambda = 0.1, gammas = 0.1, maxt = 20, x = vars,
                betas = c(tb.infection = 0.7, age = 0.03, sex = -1.9, comorbidity = 0.15))
...
# Follow-up time
simdat$follow_up <- round(simdat$follow_up, 1)
simdat$follow_up[simdat$follow_up == 0] <- 0.1
head(simdat)

# Recoding
simdat$tb.infection <- factor(simdat$tb.infection, levels = c(0, 1), labels = c("No", "Yes"))
...
# Table 1
tab1 <- CreateTableOne(c("cvd", "follow_up", "age", "sex", "comorbidity"), strata = "tb.infection", 
